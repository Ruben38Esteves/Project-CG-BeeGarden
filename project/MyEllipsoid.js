...
export class MyEllipsoid extends CGFobject {
    constructor(scene, slices, stacks, width, length, height, inverted = false) {
        super(scene);
        this.scene = scene;
        this.slices = slices;
        this.stacks = stacks;
    ...
    initBuffers() {
        ...
        for (let stack = 0; stack <= this.stacks; stack++) {
            ...
            for (let slice = 0; slice <= this.slices; slice++) {
                ...
                let x = this.width * cosPhi * sinTheta;
                let y = this.height * cosTheta;
                let z = this.length * sinPhi * sinTheta;
